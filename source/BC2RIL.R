BC2RIL <- function(P1, selected_pop, qtl_file, pop_file){
  df <- pop_file%>%
    select(-Trait)%>%
    ...
    for (chrom in 1:length(chr)){
      ...
      if (dim(p_inf_chr)[1] > 1){##染色体上存在两个或两个以上的位点
        for (i in 1:(dim(p_inf_chr)[1]-1)){
          ...
          R=2*r/(1+2*r)
          if (p_inf_chr[i, 3] == 1){ ##########################P1BC2RIL#################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P1BC2RIL代不同基因型频率如下
              ##p(AABB) = 1/8-1/8*(1-r)^2*(1-R)
              p = 1/8-1/8*(1-r)^2*(1-R)##携带两个目的基因均为纯合概率
            } else {
              ##P1=AABB, P2 = aabb, P1BC2RIL代不同基因型频率如下
              ##p(AABB) = 3/4+1/8*(1-r)^2*(1-R)
              p = 3/4+1/8*(1-r)^2*(1-R)##携带两个目的基因均为纯合的概率
            }
          } else {  ###################P2BC2RIL##################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P2BC2RIL代不同基因型频率如下
              ##p(AABB) = 1/8-1/8*(1-r)^2*(1-R)
              p = 1/8-1/8*(1-r)^2*(1-R)##携带两个目的基因均为纯合概率
            } else {
              ##P1=AABB, P2 = aabb, P2BC2RIL代不同基因型频率如下
              ##p(AABB) = 1/8*(1-r)^2*(1-R)
              p = 1/8*(1-r)^2*(1-R)##携带两个目的基因均为纯合的概率
            }
          }
          mm[nn] = p
          nn = nn + 1
  ...
  names(data_pop_size) = c("Name", "NminBC2RIL")
  write.csv(data_pop_size, paste("NminBC2RIL", "_population_size.csv", sep = ""), quote = F, row.names = F)
}
