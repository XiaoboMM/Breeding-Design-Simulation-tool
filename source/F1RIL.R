F1RIL <- function(P1, selected_pop, qtl_file, pop_file){
  df <- pop_file%>%
    select(-Trait)%>%
      ...
      if (dim(p_inf_chr)[1] > 1){##染色体上存在两个或两个以上的位点
        for (i in 1:(dim(p_inf_chr)[1]-1)){
          ...
          if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
            ...
            ##P1=AAbb, P2 = aaBB, F1RIL代不同基因型频率如下
            ##p(AABB) = 1/2*R = 1/2*(2*r/(1+2r));R=2*r/(1+2*r)
            p = 1/2*(2*r/(1+2*r))##携带两个目的基因均为纯合概率
          } else {
            r = 1/2*(1-exp(-Dis/50))###重组率
            ##P1=AABB, P2 = aabb, F1RIL代不同基因型频率如下
            ##p(AABB) = 1/2*(1-R)=1/2*(1-(2*r/(1+2*r)))
            p = 1/2*(1-(2*r/(1+2*r))) ##携带两个目的基因均为纯合或杂合的概率
          }
          mm[nn] = p
          nn = nn + 1
          f = f*p
  ...
  names(data_pop_size) = c("Name", "NminF1RIL")
  write.csv(data_pop_size, paste("NminF1RIL", "_population_size.csv", sep = ""), quote = F, row.names = F)
