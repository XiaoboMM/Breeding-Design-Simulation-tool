BC1F2enrichmentDH <- function(P1, selected_pop, qtl_file, pop_file){
  df <- pop_file%>%
    select(-Trait)%>%
  ...
  ######计算NminPBC1F2enrichmentDH，计算PBC1F2enrichmentDH最小群体大小
  data_pop_size <- data.frame(stringsAsFactors = F)
  for (trait in (1: dim(dat)[1])){
    ##############计算PBC1F2enrichmentDH最小群体大小
    ...
    for (chrom in 1:length(chr)){
      ...
      if (dim(p_inf_chr)[1] > 1){##染色体上存在两个或两个以上的位点
        for (i in 1:(dim(p_inf_chr)[1]-1)){
          ...
          R=2*r/(1+2*r)
          if (p_inf_chr[i, 3] == 1){ ##########################P1BC1F2enrichmentDH#################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, PBC1F2enrichmentDH代不同基因型频率如下
          ...
          } else {  ###################P2BC1F2enrichmentDH##################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P2BC1F2enrichmentDH代不同基因型频率如下
        ...
        if (p_inf_chr[1, 3] == 1){ ##########################P1BC1F2enrichmentDH#################
          p = 6/7
        } else { ##################P2BC1F2enrichmentDH#################
          p = 2/3
        }
  ...
  names(data_pop_size) = c("Name", "NminBC1F2enrichmentDH")
  write.csv(data_pop_size, paste("NminBC1F2enrichmentDH", "_population_size.csv", sep = ""), quote = F, row.names = F)
