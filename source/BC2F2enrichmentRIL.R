BC2F2enrichmentRIL <- function(P1, selected_pop, qtl_file, pop_file){
  df <- pop_file%>%
    select(-Trait)%>%
    ...
    for (chrom in 1:length(chr)){
      ...
      if (dim(p_inf_chr)[1] > 1){##染色体上存在两个或两个以上的位点
        for (i in 1:(dim(p_inf_chr)[1]-1)){
          ...
          R=2*r/(1+2*r)
          if (p_inf_chr[i, 3] == 1){ ##########################P1BC2F2enrichmentRIL#################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P1BC2F2enrichmentRIL代不同基因型频率如下
          ...
          } else {  ###################P2BC2F2enrichmentRIL##################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P2BC2F2enrichmentRIL代不同基因型频率如下
        ...
        if (p_inf_chr[1, 3] == 1){ ##########################P1BC2F2enrichmentRIL#################
          p = 14/15
        } else { ##################P2BC2F2enrichmentRIL#################
          p = 2/3
        }
  ...
  names(data_pop_size) = c("Name", "NminBC2F2enrichmentRIL")
  write.csv(data_pop_size, paste("NminBC2F2enrichmentRIL", "_population_size.csv", sep = ""), quote = F, row.names = F)
