BC1DH <- function(P1, selected_pop, qtl_file, pop_file){
  df <- pop_file%>%
    select(-Trait)%>%
    ...
    for (chrom in 1:length(chr)){
      ...
      if (dim(p_inf_chr)[1] > 1){##染色体上存在两个或两个以上的位点
        for (i in 1:(dim(p_inf_chr)[1]-1)){
          ...
          if (p_inf_chr[i, 3] == 1){ ##########################P1BC1DH#################
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P1BC1DH代不同基因型频率如下
            ...
            if (p_inf_chr[i, 3] != p_inf_chr[i+1, 3]){
              ##P1=AAbb, P2 = aaBB, P2BC1DH代不同基因型频率如下
              ##p(AABB) = 1/4-1/4*(1-r)^2
              p = 1/4-1/4*(1-r)^2##携带两个目的基因均为纯合概率
            } else {
              ##P1=AABB, P2 = aabb, P2BC1DH代不同基因型频率如下
  ...
  names(data_pop_size) = c("Name", "NminBC1DH")
  write.csv(data_pop_size, paste("NminBC1DH", "_population_size.csv", sep = ""), quote = F, row.names = F)
